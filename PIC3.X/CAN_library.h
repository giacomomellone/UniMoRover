// CAN Library for PIC #3. 
// This PIC is used to send data via CAN to PIC #2 which receives data and establishes the presence of an obstacle in the front of the vehicle.
// The Identifier of the message is 2, which corresponds to the message containing the data of the front ultrasonic sensors.
// PIC #3 doesn't receive messages from CAN Bus.
...
// Transmission of the CAN message with ID = 2 which is located on TX Buffer 0
void CAN_TX_write_1(unsigned char data){
    ...
    TXB0SIDH = 0x00;                            
    TXB0SIDL = 0x40;                                // Identifier of message = 2 (010)
    ...
    while (TXB0CONbits.TXREQ != 0) {                // Wait for the CAN TX buffer to be ready
        ...
        if (TXB0CONbits.TXERR == 1){                // Check if there is a bus error in the transmission of the CAN message
